 ...
 * MiniGolfv1_2
 */
public class aiDialog implements Screen{
    protected MiniGolfExecutor game;
    private Stage stage;
    private int hits;
    private int winner;

    public aiDialog (final MiniGolfExecutor game, int hits,int winner){
        this.game = game;
        this.hits = hits;
        this.winner = winner;
        initializeAIdialog();
    ...
    public void initializeAIdialog(){
        stage = new Stage(new ScreenViewport());
        Skin skin = new Skin(Gdx.files.internal( PathUtil.assetsPath()+"uiskin.json"));

        Label textButton;
        if(winner == 1){
            textButton = new Label("You won with " + hits + " hits", skin, "default");
        }
        else{
            textButton = new Label("AI won with " + hits + " hits", skin, "default");
        }
